...
public class Processor {
	...
	// Attributes
	static String pathToGetAttr = System.getProperty("user.home") + "/workspace/sentiment-analysis-twitter/";
	static String main_folder = pathToGetAttr + "resources/";		// the path to the "resources" folder 
	static String test_dataset = "Liebherr";						// available datasets are goethe, Liebherr, and Cisco
	static boolean useSlidingWindowForTraining = false;				// if set to "true", only the last 1000 documents will be used for the training of the ensemble classifier
	...
	public static void startProcessor() throws Exception {
		...
		// initiate tweetText with the extracted tweets (text only)
		tweetText = Tweet.qrTweets_Text;
		
		// create object SentimentAnalyser if it has not been created previously
		if (analyser != null) {
	...
	/* DEBUGGING PURPOSE - ADD THE TWEET TEXT MANUALLY (not fetched directly from Twitter)
